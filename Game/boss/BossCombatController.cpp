...
struct BossCombatController::Impl {
    ...
    // Free-swimming marine creatures affected by the screw suction.
    // Keep a small per-creature velocity here so the pull feels continuous
    // without changing Debris' normal floating/swimming implementation.
    std::vector<Debris*> screwSuctionCreatures;
    std::unordered_map<Debris*, Vector3> screwSuctionVelocity;

    static constexpr float kCreatureRadius = 3.5f; // Same radius as thrown-item contacts.

    static bool IsScrewSuctionCreature(const Debris& creature) {
        if (creature.IsDead() || creature.GetState() != DebrisState::Floating ||
            !Collision::Finite(creature.GetPosition())) {
            return false;
        }

        // Artificial debris is not a marine creature.
        return creature.GetType() != DebrisType::DrumCan &&
            creature.GetType() != DebrisType::Screw;
    }
...
void BossCombatController::Reset(Player* player) {
    ...
    e.screwSuctionCreatures.clear(); e.screwSuctionVelocity.clear();
    ...
    for (auto& mine : mines) {
        ...
        for (const auto& [creature, target] : creatureTargets) {
            if (CanDamageCreature(*creature) && Collision::SegmentSphere(target.from - old, target.to - now, {},
                kCreatureRadius + meta.triggerRadius)) {
                close = true; break;
            }
        }
...
void BossCombatController::Impl::ResolveAttacks(float dt, Player& player, const Vector3& from, const Vector3& to,
    ...
    // Apply the same screw gather field to free-swimming marine creatures.
    // Attached equipment and thrown objects never enter screwSuctionCreatures.
    if (screw.IsGathering()) {
        for (Debris* creature : screwSuctionCreatures) {
            if (!creature || !IsScrewSuctionCreature(*creature)) continue;

            const Vector3 position = creature->GetPosition();
            if (!screw.IsWithinSuctionRange(position)) {
                screwSuctionVelocity.erase(creature);
                continue;
            }

            auto& velocity = screwSuctionVelocity[creature];
            const Vector3 acceleration = screw.CalculateGatherForce(position, velocity);
            velocity = velocity + acceleration * dt;

            // Mild water resistance prevents an accumulated velocity spike while
            // still letting nearby creatures get dragged strongly into the screw.
            const float damping = std::max(0.0f, 1.0f - 1.5f * dt);
            velocity = velocity * damping;
            creature->SetPosition(position + velocity * dt);
        }
    } else {
        // Once the gather phase ends, creatures immediately return to their
        // normal Debris::UpdateFloating movement on the following frame.
        screwSuctionVelocity.clear();
    }
...
void BossCombatController::Update(float dt, Player& player, const Vector3& arenaCenter, bool enabled, float groundY,
    ...
    // Build screw suction targets from every live, free-swimming marine creature.
    // This intentionally includes both catchable creatures and strong creatures,
    // regardless of whether they currently have combat HP.
    e.screwSuctionCreatures.clear();
    e.screwSuctionCreatures.reserve(creatures.size());
    std::unordered_set<Debris*> activeSuctionCreatures;
    for (const auto& creature : creatures) {
        if (!creature || !Impl::IsScrewSuctionCreature(*creature)) continue;
        auto* ptr = creature.get();
        e.screwSuctionCreatures.push_back(ptr);
        activeSuctionCreatures.insert(ptr);
    }
    for (auto it = e.screwSuctionVelocity.begin(); it != e.screwSuctionVelocity.end();) {
        if (activeSuctionCreatures.contains(it->first)) { ++it; continue; }
        it = e.screwSuctionVelocity.erase(it);
    }
